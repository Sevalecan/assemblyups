...
--/c remote.call("space-exploration", "robot_attrition_for_surface", {surface_index = surface_index})
        robot_attrition_for_surface = function(data)
          local zone = Zone.from_surface_index(data.surface_index)

          if zone then
            return zone.bot_attrition
          else
            return data.default_rate or settings.global["robot-attrition-factor"].value
          end
        end,
...
--/c remote.call("space-exploration", "cancel_entity_creation", {entity=game.player.selected, player_index=1, message={"space-exploration.construction-denied"}}, event)
        cancel_entity_creation = function(data)
          return cancel_entity_creation(data.entity, data.player_index, data.message, data)
        end,
...
--/c remote.call("space-exploration", "force_add_planet_or_moon", { zone_name="Arendel"} )
        force_add_planet_or_moon = function(data)
          if global.zones_by_name[data.zone_name] then
            game.print("Planet or moon was already created.")
            return
          end
          ...
          if not planet_or_moon then
            game.print("Planet or moon does not exist. "..data.zone_name)
            return
          end
          local star = nil
          ...
          if moon_count < 5 then
            ...
            game.print(new_moon.name .." added as a moon of "..parent_planet.name.." in the "..star.name.." system.")
          else -- add to star
            ...
            new_planet.radius_multiplier = 0.4 + 0.6 * math.pow(math.random(), 2)
            ...
            game.print(new_planet.name .." added as a planet in the "..star.name.." system.")
          end
...
-- jetpack implementation
        ...
        ---@param data table {mod=<string>, entity=<LuaEntity>, ignore_count=<uint|nil>}
