...
function MapView.gui_update(player)
  ...
  if root and root[MapView.name_details_root] then
    ...
    local zone_content_frame = details[Zonelist.name_zone_content_frame]
    local zone_content_pane = zone_content_frame[Zonelist.name_zone_content_pane]
    local zone_content_preview_frame = details[Zonelist.name_zone_content_preview_frame]
    local zone = MapView.gui_get_zone_from_element(details.parent)
    Zonelist.gui_update_zone_content(player, zone_content_pane, zone_content_preview_frame, zone)
  end
end
  ...
  if event.player_index
    ...
    and game.players[event.player_index].connected then
      if event.prototype_name == MapView.name_shortcut then
        return MapView.starmap_view_cycle(game.players[event.player_index])
      end
  end
end
