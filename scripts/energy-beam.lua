...
---@param target_zone AnyZoneType|SpaceshipType Target zone
---@param coordinates Position Target coordinates
...
function EnergyBeam.set_target(tree, player, target_zone, coordinates)
  ...
  if target_zone then
    if EnergyBeam.is_reachable_destination(tree, target_zone) then
      ...
      if player then
        player.print({"space-exploration.energy-transmitter-coordinates-set",
          math.floor(coordinates.x), math.floor(coordinates.y)})
      end

      is_successful = true
      Log.trace("Emitter destination set to: " .. target_zone.name )
    else
      -- Inform the player that the chosen target_zone was not reachable
      if player then
        local source_str = "[img=" .. Zone.get_icon(tree.zone) .. "] " .. tree.zone.name
        local dest_str = "[img=" .. Zone.get_icon(target_zone) .. "] " .. target_zone.name
        player.print({"space-exploration.energy-transmitter-invalid-destination", source_str, dest_str})
      end

      is_successful = false
      Log.trace("Attempted to set unreachable emitter destination: " .. target_zone.name)
    end
  else
  ...
  if (event.item == EnergyBeam.name_transmitter_targeter) then
    ...
    if playerdata.remote_view_activity and playerdata.remote_view_activity.type == EnergyBeam.name_target_activity_type then
      ...
      if tree.emitter and tree.emitter.valid then
        ...
        local is_successful = EnergyBeam.set_target(tree, player, Zone.from_surface(player.surface), coordinates)
        if tree.mode == "energise" and is_successful then
          EnergyBeam.clear_end_beam(tree)
        end
      end
    end
...
function EnergyBeam.on_entity_created(event)
  ...
  if entity.name == EnergyBeam.name_chamber
    or entity.name == EnergyBeam.name_injector
    or entity.name == EnergyBeam.name_emitter then
    ...
    if zone and zone.type ~= "spaceship" then
      ...
      if tree and entity.name == EnergyBeam.name_emitter
      ...
      cancel_entity_creation(entity, event.player_index, {"space-explortion.construction-denied"}, event)
      return
    end
  end
...
---@param event on_player_setup_blueprint
