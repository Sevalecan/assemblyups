  ...
  elseif event.entity.name == mod_prefix.."cryogun-trigger" then
    ...
    for _, entity in pairs(freeze_entities) do
      if entity.active then
        entity.active = false
        if entity.stickers and #entity.stickers > 0 then
...
function Weapon.chain_beam(tick_task)
  ...
  if chosen then
    ...
    local bonus_damage = Shared.tesla_base_damage * force.get_ammo_damage_modifier(Shared.tesla_ammo_category)
    if bonus_damage > 0 then
      if tick_task.instigator.valid and tick_task.instigator.surface == chosen.surface then
        chosen.damage(bonus_damage, force, "electric", tick_task.instigator)
      else
