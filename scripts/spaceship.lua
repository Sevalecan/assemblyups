...
---Lands a spaceship.
---@param spaceship SpaceshipType Spaceship data
---@param position Position Position at which to land
    ...
    if event.tiles then
      ...
      if zone then
        for _, old_tile_and_position in pairs(event.tiles) do
          if Spaceship.is_floor(old_tile_and_position.old_tile.name) then
...
---@param spaceship SpaceshipType Spaceship data
---@param start_position Position "{x=int, y=int}"
...
function Spaceship.spaceship_tick(spaceship)
  ...
  if spaceship.circuits_to_restore and spaceship.circuits_to_restore_tick < game.tick then
    ...
    for _, storedState in pairs(spaceship.circuits_to_restore) do
      if storedState.entity and storedState.entity.valid then
        if spaceship.circuits_restore_phase == 1 then
          -- First tick
          local behavior = storedState.entity.get_control_behavior()
          local signal_idx = 1
  ...
  if Spaceship.is_on_own_surface(spaceship) or (spaceship.console and spaceship.console.valid) then
    ...
    if spaceship.console and spaceship.console.valid and (game.tick + spaceship.console.unit_number) % 60 == 0 then
      ...
      if not spaceship.own_surface_index and not spaceship.entities_to_restore then
        local value = util.signal_from_wires(red, green, Spaceship.signal_for_launch)
        if value > 0 then
...
function Spaceship.stress_for_generator_prototype(prototype)
  ...
  local max_power = prototype.max_energy_production
  ...
  --Log.trace("generator_cost "..prototype.name.." "..cost)
...
function Spaceship.calculate_integrity_stress(spaceship, area)
  ...
  if #overweight_generator_names > 0 then
    -- Generators
    ...
    for _, entity in pairs(entities) do
      local tile_pos = Util.position_to_tile(entity.position)
      if area == nil or (spaceship.known_tiles[tile_pos.x] and spaceship.known_tiles[tile_pos.x][tile_pos.y] == Spaceship.tile_status.floor_console_connected) then
        spaceship.integrity_stress_structure = spaceship.integrity_stress_structure + Spaceship.stress_for_generator_name(entity.name)
        spaceship.integrity_stress_container = spaceship.integrity_stress_container + Spaceship.stress_for_generator_name(entity.name)
      end
    end
  end
...
function Spaceship.on_init(event)
    ---Table of all spaceship data objects, indexed by `spaceship_index`
    ---@type table<uint, SpaceshipType>
    global.spaceships = {}
end
