  ...
  if entity.name == DAnchor.name_structure then
    ...
    if zone.parent and zone.parent.type == "star" then
      if entity.surface.count_entities_filtered{name = DAnchor.name_structure} > 1 then
        cancel_entity_creation(entity, event.player_index, {"space-exploration.dimensional_anchor_limit_1"}, event)
        return
      end
      ---@type table<uint, DimensionalAnchorInfo>
      ...
      cancel_entity_creation(entity, event.player_index, {"space-exploration.dimensional_anchor_place_on_star"}, event)
      return
    end
  end
