...
---@param launch_pad RocketLaunchPadInfo launch pad data
function LaunchpadGUI.gui_open(player, launch_pad)
  ...
  local list, selected_index, values = Zone.dropdown_list_zone_destinations(player, struct.force_name, destination_zone, playerdata.zones_alphabetical, nil,
...
function LaunchpadGUI.focus_search(event)
  local root = game.players[event.player_index].gui.relative[LaunchpadGUI.name_rocket_launch_pad_gui_root]
...
function LaunchpadGUI.gui_update_destinations_list(player)
  ...
  if root then
    ...
    local list, selected_index, values = Zone.dropdown_list_zone_destinations(player, struct.force_name, destination_zone, playerdata.zones_alphabetical, filter,
      {list = {"space-exploration.any_landing_pad_with_name"}, value = {type = "any"}} -- wildcard
    )
    if selected_index == 1 then selected_index = 2 end
