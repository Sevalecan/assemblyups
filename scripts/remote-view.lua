...
---@param event on_player_pipette Event data
...
function RemoteView.persist_inventory(player)
  ...
  playerdata.navsat_inventory = playerdata.navsat_inventory or game.create_inventory(#inventory)
  ...
  for i = 1, util.min(#inventory, #playerdata.navsat_inventory) do
    playerdata.navsat_inventory[i].set_stack(inventory[i])
...
function RemoteView.restore_inventory(player)
  ...
  for i = 1, util.min(#inventory, #playerdata.navsat_inventory) do
    inventory[i].set_stack(playerdata.navsat_inventory[i])
...
function RemoteView.check_cutscene_end_next_tick(event)
  ...
  if playerdata.remote_view_active then
    local tick_task = new_tick_task("restore-inventory-on-cutscene-exit")
...
function RemoteView.start (player, zone, position, location_name, freeze_history)
  ...
  if not RemoteView.is_unlocked(player) then return player.print({"space-exploration.remote-view-requires-satellite"}) end
  if zone and zone.surface_index ~= player.surface.index then
    if not RemoteView.is_intersurface_unlocked(player) then return player.print(RemoteView.intersurface_unlock_requirement_string(player)) end
  end
  ...
  if button_flow then
    if settings.get_player_settings(player)[RemoteView.name_setting_overhead_satellite].value == true then
      ...
      if RemoteView.is_unlocked(player) then
        ...
        button_flow[RemoteView.name_button_overhead_satellite].tooltip = {"space-exploration.remote-view-requires-satellite"}
      end
    else
