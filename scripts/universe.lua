...
function Universe.build ()
  ...
  add_zone(zone_index, zones_by_name, protouniverse.anomaly)
  ...
  for s, star in pairs(protouniverse.stars) do
    ...
    add_zone(zone_index, zones_by_name, star)
    add_zone(zone_index, zones_by_name, star.orbit)
    ...
    for p, planet in pairs(star.children) do
      ...
      add_zone(zone_index, zones_by_name, planet)
      ...
      if planet.type == "asteroid-belt" then
        ...
        add_zone(zone_index, zones_by_name, planet.orbit)
        ...
        for m, moon in pairs(planet.children) do
          ...
          add_zone(zone_index, zones_by_name, moon)
          add_zone(zone_index, zones_by_name, moon.orbit)
        end
      end
    end
  end
  ...
  for z, zone in pairs(protouniverse.space_zones) do
    ...
    add_zone(zone_index, zones_by_name, zone)
  ...
  if protouniverse then
    ...
    ---@type table<string, AnyZoneType>
    global.zones_by_name = zones_by_name
    ---@type table<LuaSurface, AnyZoneType>
...
function Universe.load_resource_data()
  ...
  -- gather any mod-specific overrides
...
function Universe.list_core_fragments()
  ...
  local prototypes = game.get_filtered_item_prototypes{{filter="subgroup", subgroup="core-fragments"}}
...
---Updates a given zone's minimum threat if it contains vitamelange
