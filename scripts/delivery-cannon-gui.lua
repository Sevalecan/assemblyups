...
function DeliveryCannonGUI.gui_open(player, delivery_cannon)
  ...
  local filter_button = filter_container.add{ type = "sprite-button", name="clear_filter", sprite="se-search-close-white",
...
function DeliveryCannonGUI.focus_search(event)
  local root = game.players[event.player_index].gui.relative[DeliveryCannonGUI.name_delivery_cannon_gui_root]
...
function DeliveryCannonGUI.gui_update_destinations_list(player)
  ...
  if root and root.tags then
    ...
    -- update the list
    local destination_zone = delivery_cannon.destination.zone
    local list, values, selected_index
