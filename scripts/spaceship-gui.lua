...
function SpaceshipGUI.gui_open_panel_destination(player, spaceship, flow_destination)
  ...
  local list, selected_index, values = Zone.dropdown_list_zone_destinations(player, spaceship.force_name, destination_zone, playerdata.zones_alphabetical)
...
function SpaceshipGUI.focus_search(event)
  local root = game.players[event.player_index].gui.relative[SpaceshipGUI.name_spaceship_gui_root]
...
function SpaceshipGUI.on_gui_click(event)
  ...
  elseif element.name == "button_anchor" then
    ...
    if spaceship.near and spaceship.near.type == "zone" and not spaceship.entities_to_restore then
      if playerdata.anchor_scouting_for_spaceship_index then
        if game.tick <= (spaceship.surface_lock_timeout or 0) or spaceship.is_doing_check then
  ...
  elseif element.name == "spaceship-list-zones" then
    ...
    if type(value) == "table" then
      if value.type == "zone" then
        ...
        Log.trace(Spaceship.get_distance_to_destination(spaceship))
...
function SpaceshipGUI.gui_update_destinations_list(player)
  ...
  if root then
    ...
    if panel then
      ...
      local list, selected_index, values = Zone.dropdown_list_zone_destinations(player, spaceship.force_name, destination_zone, playerdata.zones_alphabetical, filter)
