...
function Blueprint.update_blueprint(bp, player)
  if not player then Log.trace("no player", {"blueprint"}) end
  ...
  if entities and next(entities) then
    ...
    if offset then
      ...
      if origin.x ~= 0 or origin.y ~= 0 then
        if is_debug_mode then Log.trace("Moving blueprint offset by {"..origin.x..", "..origin.y.."} tiles.", {"blueprint"}) end
        ...
        if grid_violated then
          Log.trace("WARNING: This blueprint has clamps aligned to different grids.", {"blueprint"})
        end
        ...
        if is_debug_mode then Log.trace("Origin already set correctly", {"blueprint"}) end
      end
      ...
      if not grid or grid.x % 2 == 1 or grid.y % 2 == 1 then
        if is_debug_mode then Log.trace("Forcing blueprint grid to 2,2", {"blueprint"}) end
        bp.blueprint_snap_to_grid = {x=2, y=2}
      elseif is_debug_mode then
        Log.trace("Grid already set correctly", {"blueprint"})
      end

      if (not relative) or relative.x % 2 == 1 or relative.y % 2 == 1 then
        if is_debug_mode then Log.trace("Forcing blueprint relative offset to 0,0", {"blueprint"}) end
        bp.blueprint_position_relative_to_grid = {x = 0, y = 0}
      elseif is_debug_mode then
        Log.trace("Relative offset already set correctly", {"blueprint"})
      end

      bp.blueprint_absolute_snapping = true
      if is_debug_mode then Log.trace("Finished making blueprint snap to Spaceship Clamp.", {"blueprint"}) end

    else
      if is_debug_mode then Log.trace("No Spaceship Clamp found in Blueprint.", {"blueprint"}) end
    end

  elseif snap_clamps then
    if is_debug_mode then Log.trace("Blueprint is empty.", {"blueprint"}) end
  end
end
...
function Blueprint.on_player_setup_blueprint(event)
  ...
  if item and item.valid_for_read then
    if is_debug_mode then Log.trace("on_player_setup_blueprint is processing player.blueprint_to_setup", {"blueprint"}) end
    Blueprint.update_blueprint(item, game.players[event.player_index])
  end
end
...
function Blueprint.on_gui_closed(event)
  ...
  if item and item.valid_for_read and item.is_blueprint then
    if is_debug_mode then Log.trace("on_gui_closed is processing event.item", {"blueprint"}) end
    Blueprint.update_blueprint(item, game.players[event.player_index])
  end
end
...
function Blueprint.on_player_cursor_stack_changed(event)
  ...
  if item and item.valid_for_read then
    if item.is_blueprint then
      if is_debug_mode then Log.trace("on_player_cursor_stack_changed is processing player.cursor_stack", {"blueprint"}) end
      Blueprint.update_blueprint(item, game.players[event.player_index])
    elseif item.is_blueprint_book then
      local active_index = item.active_index
      if active_index then
        if is_debug_mode then Log.trace("on_player_cursor_stack_changed is processing blueprint book in cursor", {"blueprint"}) end
        local book_inventory = item.get_inventory(defines.inventory.item_main)
        if is_debug_mode then Log.trace("on_player_cursor_stack_changed is looking for stack #"..active_index.." in array of "..#book_inventory.." items", {"blueprint"}) end
        if active_index <= #book_inventory then
          ...
          if blueprint and blueprint.valid_for_read and blueprint.is_blueprint then
            if is_debug_mode then Log.trace("on_player_cursor_stack_changed is processing player.cursor_stack["..active_index.."]", {"blueprint"}) end
            Blueprint.update_blueprint(blueprint, game.players[event.player_index])
          end
        end
      end
    end
